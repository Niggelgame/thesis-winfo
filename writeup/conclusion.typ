...
The resulting model shows an high accuracy of *91.07%* for single next step prediction, and when considering the two possible next steps with the highest probabilities, our model reaches *100%* accuracy on the validation data set.
...
We did not focus on extracting the parameters for specific actions, such as the amount of time the MILL should perform its action. Adding these kind of parameters to the modelling and encoding could open up the prediction of more fine-grained prediction within the APS. This is a clear limitation for what kind of processes can be modelled by the infrastructure at the moment, as parameters need to be included into the tokens directly. Adding byte-pair instead of doing a 1-1 step to token mapping could simplify this in future work.

We perform no comparison to other prediction-model architectures, as this would be out of scope for this thesis. However, due to the basic simplicity of the factory, simpler traditional machine learning models could already perform quite well.

Some further actions on the basis of our model could be to

- Extend our discrete pipeline into one continious live algorithm, that can consume the live logs as an input and output concrete predictions as an output. This should be straight-forward, as the singular steps of the pipeline already exist, however the current pipeline would need to be adjusted to combine all scripts into one module.
