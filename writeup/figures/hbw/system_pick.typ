...
#figure(caption: "HBW Pick model (Store into HBW)", placement: none)[#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[system *HBW pick*]
    )
    ...
    content((3, 1.5), name: "step1", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[HBW \ Pick ])

    content((6, 1.5), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[HBW \ Picking])

    content((9, 0), name: "step2", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[HBW \ Picked])

    content((9, -3), name: "step3", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[HBW Pick \ Failed])

    // 5. OUTPUTS (Right Edge)
    // Placed at X = end-x so they straddle the right border perfectly
    content((end-x, 1.5), name: "out1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Finish \ HBW \ Pick])
