...
#let steps_drill = align(center)[#scale(x: 80%, y: 80%, reflow: true)[#canvas({
      import draw: *
      ...
      content(
        ...
        anchor: "north-east", 
        padding: 0.2, 
        text(size: 11pt)[step *DRILL Drill*]
      )

      // 3. INPUTS (Left Edge)
      // Placed at X = start-x so they straddle the left border perfectly
      content((start-x, 0), name: "in1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Finish \ DRILL \ Pick ])

      // 4. INTERNAL STEPS (Middle)
      // Placed freely between start-x and end-x
      content((3, 0), name: "step1", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL \ Drill ])

      content((6, 0), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[DRILL \ Drilling])
...
#let step_success = align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[step *DRILL Drilled*]
    )

    content((6, 0), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[DRILL \ Drilling])

    content((9, 0), name: "step2", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL \ Drilled])

    // 5. OUTPUTS (Right Edge)
    // Placed at X = end-x so they straddle the right border perfectly
    content((end-x, 0), name: "out1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Start \ DRILL \ Drop])
...
#figure(caption: "DRILL Drill steps", placement: none)[
...
#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[step *DRILL Drill Failed*]
    )
    ...
    content((9,0), name: "step3", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL Drill \ Failed])
