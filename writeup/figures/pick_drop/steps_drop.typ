...
#figure(caption: "Drop steps template")[
#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[step *`MOD` Drop*]
    )

    // 3. INPUTS (Left Edge)
    // Placed at X = start-x so they straddle the left border perfectly
    content((start-x, 1.5), name: "in1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Start \ `MOD` \ Drop ])
    content((start-x, -1.5), name: "in2", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[AGV at \ `MOD`])

    // 4. INTERNAL STEPS (Middle)
    // Placed freely between start-x and end-x
    content((3, 0), name: "step1", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[`MOD` \ Drop ])

    content((6, 0), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[`MOD` \ Dropping])
...
#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[step *`MOD` Dropped*]
    )

    content((6, 0), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[`MOD` \ Dropping])

    content((9, 0), name: "step2", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[`MOD` \ Dropped])

    // 5. OUTPUTS (Right Edge)
    // Placed at X = end-x so they straddle the right border perfectly
    content((end-x, 1.5), name: "out1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Finish \ `MOD` \ Drop])
...
#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[step *`MOD` Drop Failed*]
    )
    ...
    content((9,0), name: "step3", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[`MOD` Drop \ Failed])
