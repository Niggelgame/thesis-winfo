...
Predictive process monitoring needs to face certain challenges. In modern environments, process executions are inherently concurrent and parallel. As a result, given a running process execution, *the* next event is not clearly defined: Due to the parallel execution multiple events could be executed at the same time, or in an arbitrary order depending on system load. While a linear log collected by an enterprise system might exist, due to the parallel execution this order should not be fully enforced onto predictions. 
