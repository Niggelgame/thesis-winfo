...
#figure(caption: "DRILL Perform Drilling model", placement: none)[#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content(
      ...
      anchor: "north-east", 
      padding: 0.2, 
      text(size: 11pt)[system *DRILL drill*]
    )

    // 3. INPUTS (Left Edge)
    // Placed at X = start-x so they straddle the left border perfectly
    content((start-x, 1.5), name: "in1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Finish \ DRILL \ Drill ])

    // 4. INTERNAL STEPS (Middle)
    // Placed freely between start-x and end-x
    content((3, 1.5), name: "step1", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL \ Drill ])

    content((6, 1.5), name: "place1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[DRILL \ Drilling])

    content((9, 1.5), name: "step2", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL \ Drilled ])

    content((9, -1.5), name: "step3", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[DRILL \ Drill \ Failed])

    // 5. OUTPUTS (Right Edge)
    // Placed at X = end-x so they straddle the right border perfectly
    content((end-x, 1.5), name: "out1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Start \ DRILL \ Drop])
