...
#let steps_mill = align(center)[#scale(x: 80%, y: 80%, reflow: true)[#canvas({
      ...
      content((start-x, 0), name: "in1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Finish \ AIQS \ Pick ])

      // 4. INTERNAL STEPS (Middle)
      // Placed freely between start-x and end-x
      content((3, 0), name: "step1", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[AIQS \ Check \ Quality ])
...
#let step_success = align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content((end-x, 0), name: "out1", frame: "circle", fill: white, stroke: 2pt, padding: 0.2, align(center)[Start \ AIQS \ Drop])
...
#align(center)[
  #scale(x: 80%, y: 80%, reflow: true)[#canvas({
    ...
    content((9,0), name: "step3", frame: "rect", fill: white, stroke: 2pt, padding: 0.4, align(center)[AIQS \ Check \ Failed])
